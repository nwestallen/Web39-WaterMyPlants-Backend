...
//---------------------------- ENDPOINTS ------------------------------
//[POST] /createnewuser - registers a user with hashed password XX
//[POST] /login - authenticates user and returns a token XX
...
describe('[POST] /createnewuser', () => {
  ...
  it('responds with a status code of 201 on successful registration', async () => {
    const res = await request(server).post('/createnewuser').send(newUser);
  ...
  it('adds new user to user table on success', async () => {
    await request(server).post('/createnewuser').send(newUser);
  ...
  it('responds with status code 400 if username, password, or phonenumber is missing', async () => {
    const res1 = await request(server).post('/createnewuser').send({ username: 'NoPassword', phonenumber: '555-555-5555' });
    expect(res1.status).toBe(400);
    const res2 = await request(server).post('/createnewuser').send({ username: 'NoPhone', password: 'password' });
    expect(res2.status).toBe(400);
    const res3 = await request(server).post('/createnewuser').send({ password: 'NoUser', phonenumber: '555-555-5555' });
    expect(res3.status).toBe(400);
  });

  it('responds with status code 400 if username or phonenumber is taken', async () => {
    const res1 = await request(server).post('/createnewuser').send({ username: 'OldMan', phonenumber: '123-456-7777', password: 'newpass' });
    expect(res1.status).toBe(400);
    const res2 = await request(server).post('/createnewuser').send({ username: 'NovelThompson', phonenumber: '444-444-4444', password: 'newpass'});
    expect(res2.status).toBe(400);
  });
});
...
describe('[POST] /login', () => {
  ...
  it('responds with status code 200 on successful login', async () => {
    const res = await request(server).post('/login').send(oldUser);
  ...
  it('responds with status code 400 if username or password is missing', async () => {
    const res1 = await request(server).post('/login').send({ username: 'OldMan' })
    expect(res1.status).toBe(400);
    const res2 = await request(server).post('/login').send({ password: 'password' })
    expect(res2.status).toBe(400);
  });

  it('responds with status 401 on bad credentials', async () => {
    const res1 = await request(server).post('/login').send({ username: 'OldMan', password: 'totallyFake' });
...
describe('[GET] /api/users/users', () => {
  ...
  it('responds with status 200', async () => {
    const login = await request(server).post('/login').send(oldUser);
